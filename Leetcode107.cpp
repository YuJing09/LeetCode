...
public:
    vector<vector<int>> levelOrderBottom(TreeNode* root) {
        ...
        while (temp.size() > 0) {
            ...
            for (size_t i = 0; i < size; ++i) {
                if (temp.front()->left) {
                    temp.push(temp.front()->left);
                    tempv.push_back(temp.front()->left->val);
                }
                if (temp.front()->right) {
                    temp.push(temp.front()->right);
                    tempv.push_back(temp.front()->right->val);

                }
                temp.pop();
            }
            if (tempv.size() > 0) {
                output.insert(output.begin(), tempv);
            }
    ...
    //vector<int> a{ 3,9,20,NULL,NULL,15,7 };
        ...
        Solution sol;
        vector<vector<int>> output= sol.levelOrderBottom(tree);
