...
public:
    int findMin(vector<int>& nums) {
        ...
        int size = nums.size();
        int half = size / 2;
        int leftdif = nums[half - 1] - nums[0];
        int rightdif = nums.back() - nums[half];
        int min;
        vector<int> l, r;
        if (leftdif > 0 && rightdif > 0) {
            min = nums[0] < nums[half] ? nums[0] : nums[half];
            return min;
        }
        else if (leftdif > 0 && rightdif == 0) {
            return nums[0];
        }
        else if (rightdif > 0 && leftdif == 0) {
            return nums[half];
        }
        else {
            if (leftdif < 0) {
                l.assign(nums.begin(), nums.begin() + half);
                min = findMin(l);
                return min;
            }
            else if(rightdif<0) {
                r.assign(nums.begin() + half, nums.end());
                min = findMin(r);
                return min;
            }
            else {
...
int main()
{ 
    vector<int> t{ 10,10,10,1,10 };
    
    Solution sol;

    cout << sol.findMin(t) << endl;
} 
