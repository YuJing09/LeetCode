...
public:
    int sumNumbers(TreeNode* root) {
        ...
        while (level.size() > 0) {
            current = level.front();
            currentsum = sumnumber.front();
            for (size_t i = 0; i < currentsum.size(); ++i) {
                cout << currentsum[i] << " ";
            }
            cout << endl;
            if (level.front()->left) {
                level.push(current->left);
                currentsum.push_back(current->left->val);
                sumnumber.push(currentsum);
            }

            if (current->right) {
                currentsum = sumnumber.front();
                level.push(current->right);
                currentsum.push_back(current->right->val);
                sumnumber.push(currentsum);
            }
            if (!current->left && !current->right) {
                for (size_t i = 0; i < currentsum.size(); ++i) {
                    sum += currentsum[i] * pow(10, currentsum.size()-1 - i);
                }
            }
            level.pop();
            sumnumber.pop();
