...
public:
    Node* cloneGraph(Node* node) {
        ...
        vector<Node*> clonecontainer;
        container.push_back(node);
        
        Node* output = new Node(node->val);
        clonecontainer.push_back(output);
        queue<Node*> q1;
        queue<Node*> q2;
        q1.push(node);
        q2.push(output);
        ...
        while (!q1.empty()) {
            ...
            for (size_t i = 0; i < current->neighbors.size(); ++i) {
                ...
                for (size_t j = 0; j < container.size(); ++j) {
                    if (container[j] == neighbor) {
                        current_c->neighbors.push_back(clonecontainer[j]);
                        exist = true;
                        break;
                    }
                }
                if (!exist) {
                    create = new Node(neighbor->val);
                    current_c->neighbors.push_back(create);
                    clonecontainer.push_back(create);
