...
public:
    int  maximumGap(vector<int>& nums) {
        ...
        vector<int> temp;
        vector<vector<int>> container{ 10,temp };

        int index;
        int maxn = INT_MIN;
        for (size_t i = 0; i < nums.size(); ++i) {
            index = nums[i] % 10;
            container[index].push_back(nums[i]);
            if (nums[i] > maxn) {
                maxn = nums[i];
            }
        }
        ...
        if (k != 0) {
            for (size_t i = 0; i < 10; ++i) {
                ...
                temp.insert(temp.end(), container[i].begin(), container[i].end());
            ...
            for (size_t i = 0; i < 10; ++i) {
                for (size_t j = 0; j < container[i].size(); ++j) {
                    if (temp.size() > 0) {
                        dif = abs(container[i][j] - temp.back());
                        maxgap = dif > maxgap ? dif : maxgap;
                    }
                    temp.push_back(container[i][j]);
        ...
        while (k > 0) {
            ...
            temp.clear();
            container.assign(10, temp);
            for (size_t i = 0; i < nums.size(); ++i) {
                index = (nums[i] / (int)pow(10, div)) % 10;
                container[index].push_back(nums[i]);
            }
            if (k != 1) {
                for (size_t i = 0; i < 10; ++i) {
...
int main()
{ 
    vector<int> temp{3,6,9,1};
    Solution sol;
    int out=sol.maximumGap(temp);
    cout << out << endl;
