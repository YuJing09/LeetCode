...
public:
    int maxPoints(vector<vector<int>>& points) {
        ...
        unordered_map<pair<float,float>, vector<pair<int, int>>, hash_pair> dict;
        unordered_set<pair<int, int>, hash_pair> set;
        unordered_set<pair<int, int>, hash_pair> temp;
        pair<int, int> now;
        pair<float,float> lin;
        vector<pair<int, int>> check;
        set.insert(make_pair(points[0][0], points[0][1]));
        for (size_t i = 1; i < points.size(); ++i) {
            now.first = points[i][0];
            now.second = points[i][1];
            temp = set;

            while (temp.size() > 0) {

                for (auto s : temp) {
                    lin = computelin(s, now);
                    check = dict[lin];
                    ...
                    if (check.size()) {
                        ...
                        dict[lin].push_back(now);
                        if (dict[lin].size() > max) {
                            max = dict[lin].size();
                        }
                        break;
                    }
                    else {
                        dict[lin].push_back(s);
                        dict[lin].push_back(now);
                        if (dict[lin].size() > max) {
                            max = dict[lin].size();
                        }
