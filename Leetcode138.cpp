...
public:
    Node* copyRandomList(Node* head) {
        unordered_map<unsigned int, vector<Node*>> dict;
        Node* output = new Node(head->val);
        vector<Node*> container;
        Node* current=output;
        
        unsigned int index=(unsigned int)head;
        dict[index].push_back(head);
        dict[index].push_back(current);
        while (head) {
            if (head->next) {
                index = (unsigned int)head->next;
                if (dict[index].empty()) {
                    dict[index].push_back(head->next);
                    current->next = new Node(head->next->val);
                    dict[index].push_back(current->next);
                }
                else {
                    current->next = dict[index].back();
                }
            }
            if (head->random) {
                index = (unsigned int)head->random;
                if (dict[index].empty()) {
                    dict[index].push_back(head->random);
                    current->random = new Node(head->random->val);
                    dict[index].push_back(current->random);
                }
                else {
                    current->random = dict[index].back();
                }
            }
            head = head->next;
