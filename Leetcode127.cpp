        ...
        while (paths.size() > 0) {
            ...
            for (size_t i = 0; i < current.size(); ++i) {
                ...
                for (char ch = 'a'; ch <= 'z'; ++ch) {
                    ...
                    if (!dict.count(nextpath))continue;
                    nowpath = temp;
                    nowpath.push_back(nextpath);
                    del.push_back(nextpath);
                    paths.push(nowpath);
                    if (nextpath == endWord) {
...
int main() {
    ...
    string endWord = "cog";
    vector<string> wordList{"hot", "cog", "dot","hit", "dog", "lot", "log"
};
    Solution sol;
